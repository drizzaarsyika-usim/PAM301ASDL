      ...
      <div className="bg-white w-full max-w-4xl max-h-[92vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-200 print:shadow-none print:border-none print:max-w-none print:max-h-none print:rounded-none print:overflow-visible print:block">
        ...
        <div className="p-4 bg-slate-900 text-white flex items-center justify-between print:hidden">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-sm transition-all cursor-pointer"
            >
        ...
        <div className="p-6 sm:p-8 overflow-y-auto space-y-5 text-slate-800 print:p-0 print:space-y-4 print:overflow-visible">
          
          {/* Header Block */}
          <div className="border-b-2 border-slate-900 pb-3 flex flex-col sm:flex-row sm:items-end justify-between gap-3 page-break-inside-avoid">
            <div>
              <span className="text-[10px] uppercase font-extrabold tracking-wider text-emerald-700 block">
                Clinical Reasoning Self-Directed Learning (SDL) • Student Portfolio
              </span>
            ...
            <div className="text-left sm:text-right text-xs text-slate-600 font-medium shrink-0">
          ...
          {/* Team Roles Matrix */}
          <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 text-xs page-break-inside-avoid">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {TEAM_ROLES.map((role) => (
                <div key={role.id} className="p-1.5 rounded bg-white border border-slate-200">
                  <div className="text-[9px] text-slate-500 font-bold uppercase">{role.name}</div>
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-xs text-slate-950 uppercase tracking-wider border-b border-slate-200 pb-1 page-break-inside-avoid">
            ...
            {currentCase.stages.map((stage) => {
              ...
              return (
                <div key={stage.stageKey} className="border border-slate-200 rounded-xl p-3 space-y-1.5 bg-white page-break-inside-avoid">
                  ...
                  <div className="bg-slate-50 p-2 rounded-lg border border-slate-200 text-xs text-slate-800">
                    <span className="text-[9px] uppercase font-bold text-indigo-700 block mb-0.5">
                      Team Synthesized Reasoning:
                    </span>
                    <p className="whitespace-pre-wrap font-medium text-[11px] leading-relaxed">
                      {teamText?.trim() || 'No text committed for this stage.'}
                    </p>
                  </div>

                  {/* Expert Summary Reference */}
                  <div className="p-2 rounded-lg bg-emerald-50/50 border border-emerald-100 text-xs text-emerald-950">
          ...
          <div className="border border-slate-200 rounded-xl p-3.5 space-y-2 bg-white page-break-inside-avoid">
          ...
          <div className="pt-4 border-t-2 border-slate-900 grid grid-cols-2 gap-8 text-xs page-break-inside-avoid">
            <div>
              <p className="font-bold text-slate-900">Student Scribe Signature:</p>
              <div className="h-9 border-b border-dashed border-slate-400 mt-2" />
              <p className="text-slate-500 mt-1 text-[11px]">Name: {roleAssignments['scribe'] || 'Scribe'}</p>
            </div>
            <div>
              <p className="font-bold text-slate-900">Clinical Facilitator / Faculty Sign-Off:</p>
              <div className="h-9 border-b border-dashed border-slate-400 mt-2" />
              <p className="text-slate-500 mt-1 text-[11px]">Date &amp; Stamp:</p>
            </div>
          </div>
        </div>
      </div>
